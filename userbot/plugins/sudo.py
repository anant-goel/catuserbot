...
@catub.cat_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your Anantuserbot.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your AnantUserbot."
...
async def _(event):
    ...
    try:
        ...
        sudousers = {}
    if len(sudochats) == 0:
        return await edit_delete(event, "__There are no sudo users for your Anantuserbot.__")
    result = "**The list of sudo users for your Anantuserbot are :**\n\n"
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    if input_str[0] == "-all":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += f"`{plugin}` __There is no such plugin in your AnantUserbot__.\n"
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your AnantUserbot__.\n"
    ...
    result = f"__Successfully enabled __ `{len(loadcmds)}` __ for AnantUserbot sudo.__\n"
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    result = f"__Successfully disabled __ `{count}` __ for AnantUserbot sudo.__\n"
    output = result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    if errors != "":
        output += "\n**Errors:**\n" + errors
    msg = await edit_or_reply(catevent, output)
